    ...
    const init = async () => {
      ...
      const run = timestamp => {
        ...
        if (direction === 'horizontal') {
          // Horizontal: shift each row left/right via fast GPU drawImage
          for (let j = 0; j < tightHeight; j++) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, 0, j, offscreenWidth, 1, dx, j, offscreenWidth, 1);
          }
        } else if (direction === 'vertical') {
          // Vertical: shift each column up/down via fast GPU drawImage
          ...
          // Both: shift each row horizontally using GPU drawImage
          for (let j = 0; j < tightHeight; j++) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            const dy = Math.floor(currentIntensity * (Math.random() - 0.5) * (fuzzRange * 0.35));
